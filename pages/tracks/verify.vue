<template>
  <v-container>
    <v-row class="d-flex justify-center">
      <v-col cols="4">
        <v-card>
          <v-card-title>Verify Track</v-card-title>
          <v-card-text>
            <p>Open a proposal to verify your track.... [TODO: TO COMPLETE...]</p>
            <v-text-field
              ref="id"
              v-model="id"
              label="Enter the track id"
...
export default {
  ...
  methods: {
    async onSubmit() {
      ...
      const response = await wallet.verifyTrackProposal(
        this.id,
        this.title,
        this.description,
        from,
