<template>
  <v-container>
    <v-row class="d-flex justify-center">
      <v-col cols="4">
        <v-card>
          <v-card-title>Verify Artist</v-card-title>
          <v-card-text>
            <p>
              Open a proposal to verify your artist.... [TODO: TO COMPLETE...]
            </p>
            <v-text-field
              ref="id"
              v-model="id"
              label="Enter the artist id"
...
export default {
  ...
  methods: {
    async onSubmit() {
      ...
      const response = await wallet.verifyArtistProposal(
        this.id,
        this.title,
        this.description,
        from,
